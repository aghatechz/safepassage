...
export default function Home({ setServedBy }) {
    ...
    const loadHomeData = async () => {
      try {
        ...
        setStats({
          ...
          agenciesCount: agenciesList.length,
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
      ...
      <div className="gradient-teal-subtle rounded-2xl border border-teal-100 p-6 sm:p-10 mb-8 shadow-sm">
        <div className="max-w-3xl">
          <div className="flex items-center gap-2 mb-3">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-teal-100 px-3 py-1 text-xs font-semibold text-teal-800">
              ...
              Crowd-Sourced Verification Platform
            </span>
          </div>
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-slate-900 tracking-tight leading-tight">
            Verify Overseas Job Offers.<br />
            <span className="text-teal-600">Avoid Recruitment Scams.</span>
          </h1>
          <p className="mt-3 text-base sm:text-lg text-slate-500 max-w-2xl leading-relaxed">
            Search thousands of recruitment agencies, verify their credibility, 
            and report fraudulent job offers — all in one place.
          </p>
        </div>
        ...
        <div className="mt-6 max-w-2xl">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search agency by name, location (e.g. 'Gulf', 'Mumbai', 'Dubai')..."
              className="block w-full rounded-xl border border-slate-300 bg-white py-3.5 pl-11 pr-4 text-slate-900 placeholder-slate-400 shadow-sm focus:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-500/20 transition-all text-base"
            />
          </div>
        </div>
      </div>
              ...
              {agencies.map((agency) => (
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {agency.is_verified && (
                          <span className="badge-teal text-[10px] py-0.5 shrink-0">
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    <span className={`text-xs font-bold px-2.5 py-1 rounded-md ${getScoreBadge(agency.trust_score)}`}>
                      {agency.trust_score}%
                    </span>
        ...
        <div className="lg:col-span-1">
          ...
          {loading ? (
            ...
            <div className="card p-8 text-center">
              ...
              <p className="text-sm text-slate-500">No scam reports filed yet.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {recentReports.map((report) => (
                <div
                  key={report.id}
                  className="card p-4 border-l-4 border-l-amber-400 space-y-2.5"
                >
                  <div>
                    <Link
                      to={`/agency/${report.agency_id}`}
                      className="font-semibold text-slate-900 hover:text-teal-600 transition-colors text-sm block"
                    >
                      {report.agency_name}
                    </Link>
